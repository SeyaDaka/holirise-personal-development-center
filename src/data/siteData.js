export const siteData = {
  organization: {
    name: "HOLIRISE",
    fullName: "HOLIRISE Personal Development Center",
    tagline: "Learn \u2022 Grow \u2022 Flourish",
    description:
      "HOLIRISE Personal Development Center creates practical learning, leadership and life-development opportunities that help individuals and communities move from potential to meaningful impact.",
    location: "Dessie, Ethiopia",
    website: "https://www.holirise.com",
    phones: ["+251 911 81 91 69"],
    email: "info@holirise.com",
    address: "Dessie, Ethiopia",
    director: "Director | HOLIRISE Personal Development Center",
    copyright:
      "Developed By Birhanu Asmerom \u00a9 2026 HOLIRISE Personal Development Center. All Rights Reserved.",
  },

  navLinks: [
    { label: "Home", href: "#home" },
    { label: "About", href: "#about" },
    { label: "Programs", href: "#programs" },
    { label: "Our Approach", href: "#approach" },
    { label: "Values", href: "#values" },
    { label: "Team", href: "#team" },
    { label: "Contact", href: "#contact" },
  ],

  hero: {
    label: "LEARN \u2022 GROW \u2022 FLOURISH",
    headline: "Empowering People to Flourish.",
    highlightWord: "Flourish",
    text: "HOLIRISE Personal Development Center creates practical learning, leadership and life-development opportunities that help individuals and communities move from potential to meaningful impact.",
    primaryCta: "Explore Our Programs",
    secondaryCta: "Contact Us",
    stats: [
      { value: "People", label: "at the center" },
      { value: "Skills", label: "for real life" },
      { value: "Impact", label: "for communities" },
    ],
    badges: ["Purpose-driven", "Community-focused"],
  },

  about: {
    label: "WHO WE ARE",
    heading: "A Place Where Growth Becomes a Way of Life.",
    text: "HOLIRISE Personal Development Center is a purpose-driven organization based in Dessie, Ethiopia, dedicated to holistic personal and community growth. We believe that when individuals discover their potential and develop practical skills, they create meaningful change\u2014in their own lives, their families, and their communities.",
    text2:
      "Through mentorship, training, and community-centered programs, we foster leadership, life skills, wellbeing, and lifelong learning. Our work is rooted in the conviction that every person has the capacity to grow, lead, and contribute positively to the world around them.",
    features: [
      {
        icon: "bi-person-hearts",
        title: "Mentorship",
        desc: "Guiding individuals toward self-discovery and growth.",
      },
      {
        icon: "bi-easel",
        title: "Training",
        desc: "Practical learning experiences for real-world impact.",
      },
      {
        icon: "bi-award",
        title: "Leadership",
        desc: "Developing confident, responsible leaders.",
      },
      {
        icon: "bi-people",
        title: "Community",
        desc: "Strengthening communities through empowered individuals.",
      },
    ],
  },

  vision: {
    icon: "bi-compass",
    label: "OUR VISION",
    heading:
      "A flourishing generation with purpose, skills and positive influence.",
    text: "HOLIRISE envisions individuals and communities where people understand themselves, lead responsibly, support one another and create positive solutions. We see a future where personal growth and community wellbeing go hand in hand.",
    quote:
      "Growth begins with a direction \u2014 flourishing grows through action.",
  },

  programs: {
    label: "WHAT WE DO",
    heading: "Programs Designed for Meaningful Growth",
    description:
      "We provide opportunities for young people, professionals, students, educators, families and community leaders to develop skills, confidence and purpose.",
    items: [
      {
        id: 1,
        icon: "bi-mortarboard-fill",
        title: "Personal Development",
        desc: "Practical learning experiences that build confidence, self-awareness, resilience and purposeful living.",
        cta: "Learn More",
      },
      {
        id: 2,
        icon: "bi-people-fill",
        title: "Leadership & Teamwork",
        desc: "Develop communication, collaboration, decision-making and ethical leadership skills for real-world impact.",
        cta: "Learn More",
      },
      {
        id: 3,
        icon: "bi-lightbulb-fill",
        title: "Life Skills & Innovation",
        desc: "Turn ideas into action through creativity, problem-solving, entrepreneurship and lifelong learning.",
        cta: "Learn More",
      },
      {
        id: 4,
        icon: "bi-heart-pulse-fill",
        title: "Wellbeing & Flourishing",
        desc: "Holistic activities that support healthy relationships, emotional wellbeing, positive habits and personal growth.",
        cta: "Learn More",
      },
    ],
  },

  approach: {
    label: "OUR APPROACH",
    heading: "From Inspiration to Transformation",
    description:
      "We combine reflection with practical action so participants can apply what they learn beyond the classroom.",
    steps: [
      {
        num: "01",
        title: "Discover",
        desc: "Identify strengths, needs, values and goals.",
      },
      {
        num: "02",
        title: "Develop",
        desc: "Build knowledge, habits, skills and confidence.",
      },
      {
        num: "03",
        title: "Practice",
        desc: "Apply learning through projects and real-life challenges.",
      },
      {
        num: "04",
        title: "Flourish",
        desc: "Create positive change in personal and community life.",
      },
    ],
  },

  values: {
    label: "WHAT GUIDES US",
    heading: "Values That Shape Every Experience",
    items: [
      {
        icon: "bi-compass",
        title: "Purpose",
        desc: "We help people discover direction, meaning and practical pathways for growth.",
      },
      {
        icon: "bi-heart",
        title: "Humanity",
        desc: "We put people, dignity, inclusion and community wellbeing at the center of our work.",
      },
      {
        icon: "bi-stars",
        title: "Excellence",
        desc: "We pursue high-quality learning experiences with measurable and lasting outcomes.",
      },
      {
        icon: "bi-globe",
        title: "Impact",
        desc: "We connect personal transformation with positive family, institutional and community change.",
      },
    ],
  },

  impact: [
    {
      icon: "bi-graph-up-arrow",
      title: "Personal Growth",
      desc: "Building confidence, resilience and self-awareness.",
    },
    {
      icon: "bi-people-fill",
      title: "Leadership",
      desc: "Developing responsible and effective leaders.",
    },
    {
      icon: "bi-tools",
      title: "Skills",
      desc: "Providing practical knowledge for everyday life and work.",
    },
    {
      icon: "bi-hands-heart",
      title: "Community",
      desc: "Encouraging people to contribute to positive community transformation.",
    },
  ],

  team: {
    label: "OUR PEOPLE",
    heading: "Meet the Team Behind HOLIRISE",
    description:
      "A dedicated group of educators, mentors and community builders committed to helping people learn, grow and flourish.",
    members: [
      {
        name: "Dr Seydie Mebrie",
        role: "Founder & Director",
        bio: "Leads the center's vision of holistic growth, with over a decade of experience in community development and education.",
        initials: "SM",
        photo: `${import.meta.env.BASE_URL}images/Dr Seydie Mebrie.jpg`,
        socials: [
          {
            icon: "bi-github",
            url: "https://github.com/SeyaDaka",
            label: "GitHub",
          },
          {
            icon: "bi-linkedin",
            url: "https://www.linkedin.com/in/seydie-mebrie-ali-phd-4b020840/",
            label: "LinkedIn",
          },
          {
            icon: "bi-envelope-fill",
            url: "mailto:seymeb2007@gmail.com",
            label: "Email",
          },
        ],
      },
      {
        name: "Dr Seydie Mebrie",
        role: "Programs Coordinator",
        bio: "Designs and delivers practical training in leadership, teamwork and personal development for youth and professionals.",
        initials: "DB",
        photo: `${import.meta.env.BASE_URL}images/Dr Seydie Mebrie.jpg`,
        socials: [
          {
            icon: "bi-linkedin",
            url: "https://www.linkedin.com/in/seydie-mebrie-ali-phd-4b020840/",
            label: "LinkedIn",
          },
          {
            icon: "bi-envelope-fill",
            url: "mailto:seymeb2007@gmail.com",
            label: "Email",
          },
        ],
      },
      {
        name: "Dr Seydie Mebrie",
        role: "Mentorship & Training Lead",
        bio: "Coordinates mentorship circles and life-skills workshops that help participants turn learning into confident action.",
        initials: "ST",
        photo: `${import.meta.env.BASE_URL}images/Dr Seydie Mebrie.jpg`,
        socials: [
          {
            icon: "bi-linkedin",
            url: "https://www.linkedin.com/in/seydie-mebrie-ali-phd-4b020840/",
            label: "LinkedIn",
          },
          {
            icon: "bi-envelope-fill",
            url: "mailto:seymeb2007@gmail.com",
            label: "Email",
          },
        ],
      },
      {
        name: "Dr Seydie Mebrie",
        role: "Community Engagement Officer",
        bio: "Builds partnerships with schools, institutions and local communities to extend the center's impact across Dessie.",
        initials: "YG",
        photo: `${import.meta.env.BASE_URL}images/Dr Seydie Mebrie.jpg`,
        socials: [
          {
            icon: "bi-linkedin",
            url: "https://www.linkedin.com/in/seydie-mebrie-ali-phd-4b020840/",
            label: "LinkedIn",
          },
          {
            icon: "bi-envelope-fill",
            url: "mailto:seymeb2007@gmail.com",
            label: "Email",
          },
        ],
      },
    ],
  },

  cta: {
    heading: "Ready to Grow?",
    text: "Whether you are looking for a training program, partnership, mentorship or a community initiative, we would love to hear from you.",
    button: "Start a Conversation",
  },

  contact: {
    heading: "Let\u2019s Build a Flourishing Future Together.",
    description:
      "Reach out for programs, partnerships, training, mentorship or general information.",
    phones: ["+251 911 81 91 69"],
    website: "https://www.holirise.com",
    websiteDisplay: "www.holirise.com",
    address: "Dessie, Ethiopia",
    subjects: [
      "General Inquiry",
      "Training / Program",
      "Partnership",
      "Mentorship",
      "Community Initiative",
      "Other",
    ],
  },

  footer: {
    description:
      "Learn. Grow. Flourish. Building people and communities with purpose.",
    socials: [
      {
        name: "Facebook",
        icon: "bi-facebook",
        url: "https://web.facebook.com/seydie.ali",
      },
      { name: "Telegram", icon: "bi-telegram", url: "https://t.me/seyfreedom" },
      {
        name: "LinkedIn",
        icon: "bi-linkedin",
        url: "https://www.linkedin.com/in/seydie-mebrie-ali-phd-4b020840/",
      },
      { name: "GitHub", icon: "bi-github", url: "https://github.com/SeyaDaka" },
      { name: "Instagram", icon: "bi-instagram", url: "#" },
    ],
  },
};
